...
class RC5(object):
    def __init__(self, key):
        ...
        self.blocksize = 64
        self.rounds = 20
        self.iv = bytes([(i % 256) for i in gs(8)])
        self._key = int(key, 16).to_bytes(16, "little")
...
if __name__ == '__main__':
    ...
    with open("input.txt", "rb") as infile, open("output.txt", "wb") as outfile:
        cryptor = RC5(pwd)
        cryptor.mode = "RCP"
        cryptor.encrypt_file(infile, outfile)

    with open("output.txt", "rb") as crypted, open ("encrypted.txt", "wb") as decrypted:
        cryptor = RC5(pwd)
        cryptor.mode = "RCP"
        cryptor.decrypt_file(crypted, decrypted)